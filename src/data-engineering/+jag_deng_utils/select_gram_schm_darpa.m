function [chans loss] = select_gram_schm_darpa(X, Z, mvts, maxChans, varargin)
...
% based on: C:\Users\Marta Iversen\Downloads\Scripts\gramSchmDarpa_jag.m
...
if numel(varargin)>1
    ...
    stopCrit = 'none';
    % Warning.
    % If not none, ask T. Davis for: 
    %  C:\Users\Tyler\Dropbox\Work\Code\Tasks\FeedbackDecode\resources\stopCritDarpa.m
end    
...
possChans = jag_deng_utils.select_corr_zero_mean_darpa(X,Z,1:size(X,1),corrKineThresh,size(Z,1),windowType);
